            ...
            try
            {
                using (MySqlConnection Cn = new MySqlConnection(Dat_Conexion.ObtenerConnection()))
                {
                    using (MySqlCommand Cmd = new MySqlCommand("sp_UsuarioInsertar", Cn))
                    {
                        ...
                        Cmd.Parameters.Add("?tipo_doc_id", MySqlDbType.Int32).Value = Clas_Enti.idtipdoc;
                        Cmd.Parameters.Add("?direccion", MySqlDbType.VarChar).Value = Clas_Enti.direccion;
                        Cn.Open();
                ...
                using (MySqlConnection Cn = new MySqlConnection(Dat_Conexion.ObtenerConnection()))
                {
                    using (MySqlCommand Cmd = new MySqlCommand("sp_UsuarioModificar", Cn))
                    {
                       ...
                       Cmd.Parameters.Add("?_usuario_nombre", MySqlDbType.VarChar).Value = Clas_Enti.usuario_nombre;
                        ...
                        Cmd.Parameters.Add("?_tipo_doc_id", MySqlDbType.Int32).Value = Clas_Enti.idtipdoc;
                        Cmd.Parameters.Add("?_direccion", MySqlDbType.VarChar).Value = Clas_Enti.direccion;
        ...
        public List<Ent_Usuario> Listar_Por_Perfil(Ent_Usuario Cls_Enti)
        {
            ...
            try
            {
                using (MySqlConnection Cn = new MySqlConnection(Dat_Conexion.ObtenerConnection()))
                {
                    ...
                    using (MySqlCommand Cmd = new MySqlCommand("sp_UsuarioListarPorPerfil", Cn) { CommandType = System.Data.CommandType.StoredProcedure })
                    {
                        Cmd.Parameters.Add("?_usuario_dni", MySqlDbType.Int32).Value = Cls_Enti.usuario_dni;
                        Cmd.Parameters.Add("?_perfil_id", MySqlDbType.Int32).Value = Cls_Enti.perfil_id;
                        Cn.Open();
                        using (MySqlDataReader Dr = Cmd.ExecuteReader())
                        {
                            while (Dr.Read())
